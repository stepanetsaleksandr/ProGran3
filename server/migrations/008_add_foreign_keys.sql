-- Міграція 008: Додавання Foreign Key constraints та тригерів
-- Дата: 2025-09-28
...
-- 15. Тестова вставка для перевірки роботи зв'язків
-- (Це буде виконано тільки якщо є тестові дані)
DO $$
BEGIN
  -- Перевіряємо чи є тестові ліцензії
  IF EXISTS (SELECT 1 FROM licenses WHERE license_key = 'QY0O-I01I-3CXR-5ASU') THEN
    RAISE NOTICE 'Test license found, relationships are working correctly';
  ELSE
    RAISE NOTICE 'No test licenses found, relationships will be tested when data is added';
  END IF;
END $$;
